...
export default function KooButton ({children, withIcon, icon, href, moreStyling, type, target}: KooButtonProps) {
  ...
  if (href) {
    return (
      <a href={href} target={target} className={`${baseClass} ${withIcon && "gap-x-2"} ${moreStyling}`}>
        {withIcon && icon}
        {children}
        </a>
    )
